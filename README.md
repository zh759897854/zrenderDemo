  ...
  <div class="invest-map">
    <div class="map-container" ref="svg">
      <svg id="main-svg" xmlns="http://www.w3.org/2000/svg" version="1.1">
        ...
        <defs>  
          <!-- 添加带箭头的标线 -->
          <marker id="markerArrow" markerWidth="8" markerHeight="8" refx="2" refy="5" orient="auto"></marker>
          <!-- 高斯模糊 -->
    ...
    <no-data v-show="!mapData.orgName && init" dataInfo="暂无投资族谱相关数据"></no-data>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    // 处理投资族谱数据
    computedData() {
      if (this.mapData.hasOwnProperty('child')) {
        let tmpData = Object.assign(this.mapData);
        tmpData.children = this.mapData.child || [];
        tmpData.name = this.mapData.orgName;
     ...
     * @params: addCollapse 是否收起二级之后的数据，弹出层不需要
     * @return: null
     */
    upDataMap(container, filter, curData, addCollapse) {
    ...
    drawMap(container, filter, hierarchyData) {
      ...
      for (let i = 0; i < hierarchyData.descendants().length; i++) {
        let item = hierarchyData.descendants()[i];
        nameData.push(item.data.name.length || 0)
      }
      // 获取最大宽度
      ...
      // 添加拖拽事件
      d3.select(container).call(d3.drag().on("start", function () {
        let dragBox = d3.select(container + ' g').classed("dragging", true);
        d3.event.on("drag", dragged).on("end", ended);

        let translateData = d3.select(container + ' g').attr('transform');
        let computedData = translateData.substring(10, translateData.length - 1).split(',');
        ...
        function dragged() {
          xMove += d3.event.dx;
          yMove += d3.event.dy;
          dragBox.raise().attr('transform', 'translate(' + xMove + ',' + yMove + ')');
        }
      ...
      let gs = g.append("g")
          ...
          .attr("transform", function (d) {
            ...
            // 绘制投资关系
            let circleObj = {
              circle4: [[-190, -17], [-152, -17], [-190, 17], [-152, 17]],
              circle3: [[-190, -17], [-152, -17], [-190, 17]],
              circle2: [[-190, 0], [-152, 0]],
              circle1: [[-172, 0]],
              circle0: []
            };
            ...
            if (d.depth + '' !== '0' && d.data) {
              ...
              for (let i = 0; i < relation.length; i++) {
                drawInvistCircle(that, circleObj['circle' + relation.length][i], relation[i])
              }
            }
            // 绘制展开图标
      ...
      gs.insert("rect", '.filter_circle')
          ...
          .attr("width", function (d) {
            // depth为0 是祖先级节点不需要
            return (d.depth + '') === '0' ? 0 : (d.data.name.length + 5) * 15;
          })
          .attr("height", 80)
          .attr("rx", "40")
          .attr("ry", "40")
      ...
      gs.append("rect")
          ...
          .attr("y", -40)
          .attr("width", function (d) {
            return (d.data.name.length + 2) * 15
          })
          .attr("height", 80)
          .attr("rx", "40")
          .attr("ry", "40")
      ...
      gs.append("rect")
          ...
          .attr("y", -35)
          .attr("width", function (d) {
            return (d.data.name.length + 1.5) * 15
          })
          .attr("height", 70)
          .attr("rx", "35")
          .attr("ry", "35")
      ...
      gs.append("text")
          ...
          .text(function (d) {
            if (d.depth + '' !== '0') {
              ...
              result = result.join(' ');
              result = (result.length - 4 > maxLength) ? result.substr(0, maxLength) + '...' : result;
              return result;
            } else {
              return ''
            }
      ...
      function drawCollapse(that, data) {
        ...
        that.append('line')
            ...
            .attr('transform', 'translate(' + ((data.data.name.length - 1) * 14 - 7) + ',0)');
        that.append('line')
            .attr('x1', 0)
            .attr('x2', 0)
            .attr('y1', 0)
            ...
            .attr('transform', 'translate(' + (data.data.name.length - 1) * 14 + ', -7)')
      }
      ...
      function drawInvistCircle(that, data, text) {
        that.append('circle') // 绘制模糊背景色
            ...
            .attr('stroke', '#bddbf7')
            .attr('filter', 'url(#' + filter + ')');
        that.append('circle') // 绘制底色
            .attr('cx', data[0] - 1)
            .attr('cy', data[1])
            .attr('r', 14)
            .attr('fill', '#fff')
            .attr('stroke', '#fff');
        that.append('circle') // 绘制背景色
            .attr('cx', data[0])
            .attr('cy', data[1])
            .attr('r', 14)
            .attr('fill', '#18b4de')
            .attr('stroke', '#fff');
        that.append('text') // 绘制文字
            .attr("x", data[0])
            .attr("y", data[1])
            .attr("dy", 4)
            .attr("dx", -7)
      ...
      svgContainer.attr('transform', 'translate(' + 150 + ',' + height / 2 + ')')
    },
    // 转换投资类型文字
    ...
    drawFloatMap(data) {
      ...
      if (l > 0) {
        ...
        this.showFloat = true;
        this.upDataMap('#float-svg', 'Gaussian_Blur__float', lastData[0], false);
        // 设置居中位置
        let height = document.documentElement.clientHeight;
        d3.select('#float-svg .tree-container').attr('transform', 'translate(150, ' + height / 2 + ')');
      }
    },
    // 处理悬浮数据
    getParentData(data) {
